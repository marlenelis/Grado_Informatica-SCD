...
void Filosofo( int id, int nprocesos )
{
   ...
   while(1)
   {
    ...
    if(id==0){//proceso 0 cogera el tenedor en orden contrario que el resto

      // Solicita tenedor derecho
      cout <<"Filosofo "<<id<< " coge tenedor der ..." << der << endl << flush;
      // ...
       MPI_Ssend(NULL, 0, MPI_INT, der, id, MPI_COMM_WORLD);  

         // Solicita tenedor izquierdo
      cout << "Filosofo "<<id<< " solicita tenedor izq ..." << izq << endl << flush;
      // ...
       MPI_Ssend(NULL, 0, MPI_INT, izq, id, MPI_COMM_WORLD);   
...
void Tenedor(int id, int nprocesos)
{
  ...
  while( true )
  {
    ...
    // ...
     MPI_Recv(&Filo, 1, MPI_INT, Filo, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
